...
This repository contains a project-specific implementation of [Mask2Former](https://github.com/facebookresearch/Mask2Former) for semantic segmentation of adamantinoma histopathology images. It includes the original framework together with a custom two-class training, inference, and GeoJSON export workflow for whole-slide-image (WSI) patches.
...
| Model ID | Class | Source annotation labels |
| ---: | --- | --- |
| 0 | Background | 0 |
| 1 | Bone | 1 (`bone`) and 2 (`lamellair_bone`) |
...
| Detectron2 | Commit `b599f139756bd3646a26a909caf86a1a159e53a7` |
...
| Output | Purpose |
| --- | --- |
| `model_best_foreground_f1.pth` | Best eligible checkpoint based on validation foreground F1 |
| `validation_progress.csv` | Validation loss and segmentation metrics by evaluation step |
| `validation_progress.png` | Training-progress plots |
| Periodic checkpoints and logs | Standard Detectron2 training outputs |
...
| --- | --- | --- |
| `--config-file` | `WSI_DETECTRON2_CONFIG` | Mask2Former YAML configuration |
| `--weights` | `WSI_INFERENCE_WEIGHTS` | Trained V10 checkpoint |
| `--input-dir` | `WSI_INFERENCE_INPUT` | Root containing case folders |
| `--output-dir` | `WSI_INFERENCE_OUTPUT` | Inference output root |
